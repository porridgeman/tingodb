...
function applyPush(obj,$push,tdb) {
...
function applyPushAll(obj,$pushAll,tdb) {
...
tcoll.prototype.update = function (query, doc, opts, cb) {
	...
	var $unset = doc.$unset;
	var $push = doc.$push;
	var $pushAll = doc.$pushAll;
	var $inc = doc.$inc;
	var $doc = ($set || $unset || $push || $pushAll || $inc)?null:doc;
	this._tq.add(function (cb) {	
			...
			if (res.length==0) {
				if (opts.upsert) {
					...
					if ($inc)
						applyInc($doc, $inc);
					if ($push)
						applyPush($doc,$push,self._tdb);
					if ($pushAll)
						applyPushAll($doc,$pushAll,self._tdb);
					if (_.isUndefined($doc._id))
						$doc._id = new self._tdb.ObjectID();
					...
					self._get(pos, safe.sure(cb, function (obj) {
						...
						if (!$doc) {
							...
							if ($inc)
								applyInc(udoc, $inc);
							if ($push)
								applyPush(udoc,$push,self._tdb);
							if ($pushAll)
								applyPushAll(udoc,$pushAll,self._tdb);
						}
						udoc._id = obj._id;
